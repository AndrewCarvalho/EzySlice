...
namespace EzySlice {
	...
	public sealed class Slicer {
        ...
        public static SlicedHull Slice(GameObject obj, Plane pl, TextureRegion crossRegion, Material crossMaterial) {
            return Slice(obj,null, pl, crossRegion, crossMaterial);
        }
		 ...
		 * See -> Slice(Mesh, Plane) for more info
		 */
        public static SlicedHull Slice(GameObject obj, Mesh originalMesh, Plane pl, TextureRegion crossRegion, Material crossMaterial)
        {
            ...
            return Slice(mesh, originalMesh, pl, crossRegion, crossIndex);
        }

        /**
		 * Slice the gameobject mesh (if any) using the Plane, which will generate
		 * a maximum of 2 other Meshes.
		 * This function will recalculate new UV coordinates to apply the supplied texture region over
		 * the new face in it's entirety 
        ...
        public static SlicedHull Slice(Mesh sharedMesh, Plane pl, TextureRegion region, int crossIndex) {
            return Slice(sharedMesh, null, pl, region, crossIndex);
		}

        /**
		 * Slice the gameobject mesh (if any) using the Plane, which will generate
		 * a maximum of 2 other Meshes.
         * If an original mesh is supplied, it is also sliced and it's cross section is used
         * as a reference during texturing
		 * This function will recalculate new UV coordinates and will apply a subsection of the
         * texture region to new face by comparing the new face to the face generated by the 
         * original mesh.
		 * Returns null if no intersection has been found or the GameObject does not contain
		 * a valid mesh to cut.
		 */
        public static SlicedHull Slice(Mesh sharedMesh, Mesh originalSharedMesh, Plane pl, TextureRegion region, int crossIndex)
        {
            if (sharedMesh == null)
            {
                return null;
            ...
            bool hasOriginal = originalSharedMesh != null;

            // the cross section hull is common across all submeshes
            List<Vector3> crossHull = new List<Vector3>();

            List<Vector3> originalMeshIntersectionPoints = null;
            if (hasOriginal)
            {
                originalMeshIntersectionPoints = GenerateIntersectionPoints(originalSharedMesh, pl);
            }
            SlicedSubmesh[] slices = GenerateSubmeshSlicesAndCrossHull(sharedMesh, pl, crossHull);
            ...
            for (int i = 0; i < slices.Length; i++)
            {
                ...
                if (slices[i] != null && slices[i].isValid)
                {
                    ...
                    else
                    {
                        ...
                        // TODO create parent intersection point list; factor out point list generation for cleaner code in both cases.
                        return CreateFrom(slices, CreateFrom(crossHull, pl.normal, region, originalMeshIntersectionPoints), crossIndex);
                    }
                }
            }
        ...
        /**
         * Takes a mesh and plane calculates the positions of the vertices that lie
         * on their intersection.
         * Returns an empty list if there are no intersections
         */
        private static List<Vector3> GenerateIntersectionPoints(Mesh sharedMesh, Plane pl)
        {
            ...
            {
                int[] indices = sharedMesh.GetTriangles(submeshIndex);
        ...
        /**
         * Creates and array of SlicedSubmeshes for every submesh in a provided mesh pased on the plane pl.
         * The supplied crossHull list will be cleared and populated with the intersection points of the plane and mesh
         * Returns the array of SlicedSubmeshes
         */
        private static SlicedSubmesh[] GenerateSubmeshSlicesAndCrossHull( Mesh sharedMesh, Plane pl, List<Vector3> crossHull )
        {
            Vector3[] verts = sharedMesh.vertices;
            ...
            if (crossHull.Count > 0) { crossHull.Clear(); }
            ...
            for (int submeshIndex = 0; submeshIndex < submeshCount; submeshIndex++)
            {
                ...
                SlicedSubmesh submesh = new SlicedSubmesh();
                ...
                for (int index = 0; index < indicesCount; index += 3)
                {
                    ...
                    if (newTri.Split(pl, result))
                    {
                        ...
                        {
                            submesh.upperHull.Add(result.upperHull[i]);
                        }

                        for (int i = 0; i < lowerHullCount; i++)
                        {
                            submesh.lowerHull.Add(result.lowerHull[i]);
                        }
                    ...
                    else
                    {
                        ...
                        {
                            submesh.upperHull.Add(newTri);
                        }
                        else
                        {
                            submesh.lowerHull.Add(newTri);
                        }
                    }
                }

                // register into the index
                slices[submeshIndex] = submesh;
            }
        ...
        private static List<Triangle> CreateFrom(List<Vector3> intPoints, Vector3 planeNormal, TextureRegion region, List<Vector3> parentIntPoints)
        {
            List<Triangle> tris;
            Triangulator.SlicedFaceProjection parentFace;
            if (!Triangulator.MonotoneChain(parentIntPoints, planeNormal, out tris, region, out parentFace))
            {
                // Shouldn't ever happen but just in case
                return null;
            }

            if (Triangulator.MonotoneChain(intPoints, planeNormal, out tris, region, parentFace.MinimumProjectedBoundCorner, parentFace.ProjectedBoundDimensions, out parentFace ))
            {
                return tris;
            }
